...
<nav class="font-conthrax border-primary border-b bg-gray-50/25 px-4 md:px-16 dark:bg-gray-800">
	<div class="flex w-full items-center justify-between">
		...
		>
			<div class="absolute top-1/2 left-1/2 w-5 -translate-x-1/2 -translate-y-1/2 transform">
				<span
					class={`bg-text absolute h-0.5 w-5 transform transition duration-300 ease-in-out dark:bg-gray-50 ${
						isMenuOpen ? "rotate-45 delay-200 dark:bg-gray-700" : "-translate-y-1.5"
					}`}
				></span>
				<span
					class={`bg-text absolute h-0.5 transform transition-all duration-200 ease-in-out dark:bg-gray-50 ${
	...
	<nav
		class={`bg-opacity-100 fixed top-0 left-0 z-40 flex h-screen w-full transform bg-gray-50 transition-all delay-100 duration-500 dark:bg-gray-800 ${
			isMenuOpen ? "translate-x-0 opacity-100" : "-translate-x-full opacity-0"
		}`}
	>
		<ul
			class="z-50 flex w-5/6 flex-col justify-center text-center align-middle text-gray-700 dark:text-gray-50"
		>
			{#each navItems as item (item.name)}
				<li
